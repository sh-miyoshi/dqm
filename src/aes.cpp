...
AES::AES(std::string key, unsigned int key_bit_length, const char *init_vec){
	...
	case 256:
		...
		break;
	default:
		AppLogger::Error(ToString<unsigned int>(key_bit_length) + "-bit AES is not supported in this program");
		break;
	}
...
void AES::Encrypt(std::string in_fname, std::string out_fname, bool cbc, AES::PaddingMode mode){
	...
	if( !fp_in ){
		AppLogger::Error(std::string("ERROR: cannot open file: ") + in_fname);
		return;
	}

	fopen_s(&fp_out, out_fname.c_str(), "wb");
	if( !fp_out ){
		AppLogger::Error(std::string("ERROR: cannot open file: ") + out_fname);
		return;
	}
...
void AES::Decrypt(std::string in_fname, std::string out_fname, bool cbc, AES::PaddingMode mode){
	...
	if( size == 0 ){
		fclose(fp_in);
		fclose(fp_out);
		AppLogger::Error("no readable data");
		return;
	}
	__m128i cbc_vec = iv;
	while( 1 ){
