...
void IsAbleToOpenFile(const FILE *fp, std::string position, std::string fileName){
	IsAbleToOpenFile(fp != NULL, position, fileName);
}
...
std::string ChangeViewCount(unsigned long long count){
	int ms = ( int ) ( ( float ) ( count% def::FPS ) / def::FPS ) * 100;// ミリ秒(3桁)
	count /= def::FPS;
	int s = count % 60;
	count -= s;
	count /= 60;
